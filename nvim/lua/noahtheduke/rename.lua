...
local function rename(_1_)
  ...
  local bang = _1_.bang
  if (nil == bang) then
    _G.error("Missing argument bang on fnl/noahtheduke/rename.fnl:3", 2)
  else
  end
  if (nil == name) then
    _G.error("Missing argument name on fnl/noahtheduke/rename.fnl:3", 2)
  else
  end
...
local function _7_(opts)
  if (nil == opts) then
    _G.error("Missing argument opts on fnl/noahtheduke/rename.fnl:22", 2)
  else
  end
  return rename(opts)
end
