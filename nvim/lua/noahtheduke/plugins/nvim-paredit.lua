...
local function config()
  ...
  local function _8_(err_2_auto)
    if (nil == err_2_auto) then
      _G.error("Missing argument err_2_auto on fnl/noahtheduke/plugins/nvim-paredit.fnl:6", 2)
    else
    end
  ...
  if (_2_ and _3_ and _4_ and _5_ and _6_) then
    local function insert_in_list(placement)
      if (nil == placement) then
        _G.error("Missing argument placement on fnl/noahtheduke/plugins/nvim-paredit.fnl:11", 2)
      else
      end
      local function _23_()
