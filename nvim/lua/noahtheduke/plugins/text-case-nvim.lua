...
local function config()
  ...
  local function _4_(err_2_auto)
    if (nil == err_2_auto) then
      _G.error("Missing argument err_2_auto on fnl/noahtheduke/plugins/text-case-nvim.fnl:5", 2)
    else
    end
  ...
  if _2_ then
    ...
    local function _9_(err_2_auto)
      if (nil == err_2_auto) then
        _G.error("Missing argument err_2_auto on fnl/noahtheduke/plugins/text-case-nvim.fnl:7", 2)
      else
      end
...
--[[ (config) ]]
return {utils.dep("https://github.com/nvim-telescope/telescope.nvim"), utils.dep("https://github.com/johmsalas/text-case.nvim", {config = config})}
