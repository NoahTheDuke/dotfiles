...
local function setup()
  ...
  local function _6_(err_2_auto)
    if (nil == err_2_auto) then
      _G.error("Missing argument err_2_auto on fnl/noahtheduke/plugins/alpha.fnl:5", 2)
    else
    end
  ...
  if (_2_ and _3_ and _4_) then
    ...
    local function _14_(plugin)
      if (nil == plugin) then
        _G.error("Missing argument plugin on fnl/noahtheduke/plugins/alpha.fnl:9", 2)
      else
      end
      return plugin.name
    end
